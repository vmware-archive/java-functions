...
/**
 *
 * A {@link Function} that submits a task {@link LaunchRequest} to a Data Flow server. This will check if the Data Flow
 * task platform is at capacity. If not, will submit the task launch request, otherwise it will return and log a warning message.
 *
...
public class TaskLauncherFunction implements Function<LaunchRequest, Optional<Long>> {
	 ...
	 * @return an {code}Optional<Long>{code} containing the task Id if the request is accepted or empty otherwise.
	 */
	@Override
