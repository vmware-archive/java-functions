...
public class JdbcConsumerConfiguration {
	...
	@Bean
	IntegrationFlow jdbcConsumerFlow(@Qualifier("aggregator") MessageHandler aggregator, JdbcMessageHandler jdbcMessageHandler) {
		final IntegrationFlowBuilder builder = IntegrationFlows.from(Consumer.class,
				gateway -> gateway.beanName("jdbcConsumer"));
		if (properties.getBatchSize() > 1 || properties.getIdleTimeout() > 0) {
			builder.handle(aggregator);
		}
		return builder
				.handle(jdbcMessageHandler).get();
	}
