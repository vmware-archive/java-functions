...
A `Function<Flux<Message<?>>,Flux<Void>>` which accepts incoming messages and stores the payload to a Mongo DB collection using the `ReactiveMongoDbStoringMessageHandler`.
